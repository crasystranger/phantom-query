...
function TurnBlock({
  ...
  useEffect(() => {
    ...
    setEditingSql(false);
    setExplanation(null);
  }, [turn.generated_sql, turn.edited_sql]);
  ...
  async function handleDeleteTurn() {
    if (!confirm("Delete this message and its result?")) return;
    setDeleting(true);
    try {
    ...
    <div className={`${!isFirst ? "pt-6 mt-6 border-t border-border-subtle" : ""} ${deleting ? "opacity-40 pointer-events-none" : ""}`}>
        ...
        <div className="max-w-[85%]">
          {editingQuestion ? (
            <div className="bg-panel border border-border-subtle rounded-2xl rounded-br-sm px-4 py-3 space-y-2">
              ...
              <div className="flex gap-3 items-center justify-end">
                ...
                <button
                  ...
                  disabled={savingEdit}
                  className="text-xs px-2.5 py-1 rounded-md bg-accent hover:bg-accent-hover text-white disabled:opacity-50"
                >
            ...
            <div className="bg-panel rounded-2xl rounded-br-sm px-4 py-2.5">
              <p className="text-[11px] text-faint font-medium mb-0.5">You</p>
      ...
      <div className="relative rounded-2xl rounded-tl-sm border border-border-subtle bg-panel px-5 py-4">
        ...
        <p className="text-sm text-secondary leading-relaxed mb-3">
          {turn.executed
            ? "Here's the query I ran to answer that."
            : "I've generated the query below. Review it, then run it when you're ready."}
        </p>
        ...
        <div className="rounded-lg border border-border-subtle bg-ink overflow-hidden mb-3">
          ...
          {editingSql ? (
            <textarea
              ...
              rows={5}
              spellCheck={false}
              className="w-full bg-transparent px-3 py-2.5 text-xs font-mono text-primary focus:outline-none resize-y"
              autoFocus
            />
          ) : (
        ...
        {runError && (
          <div className="flex items-center justify-between gap-3 rounded-md bg-danger/10 border border-danger/30 px-3 py-2 mb-3">
            ...
            <button
              ...
              disabled={retrying}
              className="shrink-0 text-xs px-2.5 py-1 rounded-md bg-danger/20 hover:bg-danger/30 text-danger disabled:opacity-50"
            >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <button
            ...
            disabled={explaining}
            className="flex items-center gap-1.5 px-4 py-2 text-xs font-medium rounded-md border border-border-subtle text-secondary hover:bg-hover transition-colors disabled:opacity-50"
          >
